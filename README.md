...
- **AI Assist** — configure user-supplied AI endpoints and use context-rich draft actions for card writing, latest-review repair, story editing, visual generation request previews, and visual analysis request previews. Settings stay lightweight: editable endpoint preset/base URL, API key, editable model preset/model id, and model capability toggles, with protocol and route compatibility in Advanced. Text endpoint output is validated as explicit patch proposals before applying changes through the normal editor validation and undo/draft flow.
