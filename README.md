...
The workbench URL carries panel state (`/workbench/content`) and optional skin state (`?skin=catppuccin-latte`) without forcing a hard reload, so creator sessions can be refreshed or shared directly. In-progress editor work is still saved to `localStorage` and offered for restore on reload (server-validated through `/api/editor/restore`); player previews also accept the same `skin` query so creator and player surfaces stay visually aligned. `Github Light` is the default skin, and the former dark workbench skin remains available as `Workbench` through `?skin=workbench`.
