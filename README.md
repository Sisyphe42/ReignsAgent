...
Story progression remains data-driven. Authors should express narrative evolution through card `requirements`, choice `effects.tags`, choice `effects.variables`, and optional metadata labels. Card requirements can combine tag gates, exact variable gates, and default gauge thresholds through `requirements.factions` (`faith`, `people`, `military`, `treasury` only, with `min`/`max`/`equals`). The Story workspace reads lightweight `metadata.story.groups` entries for chapters/themes/arcs/endings as an authoring organization layer only; these groups filter and explain the graph without changing core scheduling. Presentation metadata may use `metadata.presentation.gauges` to rename, describe, or hide the default four gauge displays without adding new player stats.
