...
- **PR Judgment**: The Agent decides whether a change needs a PR using standard review value: prefer PRs for user-facing features, cross-module changes, public contracts, security/privacy behavior, connector/deployment behavior, nontrivial refactors, release work, and governance changes. Skip PRs for small, low-risk maintenance when direct commits are clearer.
- **PR Size**: Group related commits into one reviewable PR by behavior or risk area. Do not create one PR per commit by default, and avoid PRs that mix unrelated concerns.
- **Direct Commits**: Small docs edits, tests, typos, formatting, mechanical cleanup, and low-risk internal maintenance may go directly to `master` after checks pass. Use `chore:` style commit messages or `chore/<slug>` branches when a temporary branch is still useful.
- **Branch Naming**: For PR work, use common prefixes such as `feature/<slug>`, `fix/<slug>`, `docs/<slug>`, `chore/<slug>`, `refactor/<slug>`, `release/<version-or-title>`, or short-lived `phase/<milestone-title>`. Do not use agent/tool prefixes.
