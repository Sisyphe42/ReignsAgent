...
describe("ReignsAgent core runtime", () => {
  it("creates a headless state with four bounded factions", () => {
    const state = createInitialState({ factions: { gauge0: 20, gauge1: 80, gauge2: 120 } });
    ...
    assert.equal(state.gameOver.faction, "gauge2");
  });

  it("normalizes legacy faction keys into neutral gauge slots", () => {
    const state = createInitialState({ factions: { faith: 20, treasury: 80 } });
    const [normalizedCard] = normalizeCards([
      card("legacy", { factions: { people: { min: 55 } } }, "accept", { factions: { military: 3 } })
    ]);
    ...
    assert.deepEqual(normalizedCard.requirements.factions, { gauge1: { min: 55 } });
    assert.deepEqual(normalizedCard.choices[0].effects.factions, { gauge2: 3 });
  });
  ...
  it("gates cards by combined tags, variables, and faction thresholds", () => {
    ...
    const grainState = createInitialState({
      ...
      variables: { openingPetition: "grain" },
      factions: { gauge1: 56, gauge3: 46 }
    });
    const lowSupportState = createInitialState({
      tags: { grainRelief: true },
      variables: { openingPetition: "grain" },
      factions: { gauge1: 54, gauge3: 46 }
    });
  ...
  it("applies choice effects, advances turns, and detects game over", () => {
    const runtime = createRuntime({
      cards: [
        card("tax", {}, "raise", {
          factions: { gauge1: -60, gauge3: 20 },
          tags: { taxed: true }
        })
      ],
      state: { factions: { gauge1: 50, gauge3: 70 } },
      rng: () => 0
    });
    ...
    assert.equal(result.state.factions.gauge1, 0);
    assert.equal(result.state.factions.gauge3, 90);
  ...
  it("models customization through low-level variables, tags, and hooks only", () => {
    const favorHook = {
      ...
      hooks: {
        ...
        on_tick({ scaleFaction, data }) {
          scaleFaction("gauge1", data.scale);
        },
        on_dismiss({ clearTag }) {
    ...
    const cards = [
      card("routine", {}, "ignore"),
      card("petition", { allTags: ["favored"] }, "grant", { factions: { gauge1: 20 } })
    ];
    ...
    assert.equal(runtime.state.factions.gauge1, 70);
    assert.equal(runtime.state.factionScales.gauge1, 0.5);
  ...
  it("serializes JSON-safe snapshots and restores hook functions from a registry", () => {
    const cards = [card("repeatable", {}, "accept", { factions: { gauge1: 10 } })];
    const runtime = createRuntime({ cards, rng: () => 0 });
    ...
    assert.equal(parsedSnapshot.activeHooks[0].id, "scale-gauge1");
    ...
    const restoredState = restoreState(parsedSnapshot, {
      hookRegistry: {
        "scale-gauge1": scalingHook(0.5).hooks
      }
    });
    ...
    assert.equal(restoredRuntime.state.factions.gauge1, 60);
    assert.equal(restoredRuntime.state.factionScales.gauge1, 0.5);
  });
  ...
  it("can restore snapshots without missing hook functions when explicitly allowed", () => {
    ...
    restoredRuntime.choose("accept");
    assert.equal(restoredRuntime.state.factions.gauge1, 60);
    assert.equal(restoredRuntime.state.factionScales.gauge1, 1);
  });

  it("restores a snapshot and continues with the same draw and choice behavior", () => {
    const cards = [
      card("first", {}, "accept", { variables: { opened: true } }),
      card("second", { variables: { opened: true } }, "accept", { factions: { gauge3: 5 } })
    ];
    const runtime = createRuntime({ cards, rng: () => 0 });
  ...
  it("records JSON-safe runtime events for draw, choice, hooks, reset, stall, and ending", () => {
    ...
    const endingRuntime = createRuntime({
      cards: [card("fall", {}, "accept", { factions: { gauge1: -60 } })],
      state: { factions: { gauge1: 50 } },
      rng: () => 0
    });
    endingRuntime.draw();
    ...
    assert.equal(endingRuntime.events.at(-1).faction, "gauge1");
  });
