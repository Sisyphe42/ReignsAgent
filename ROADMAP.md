...
- Creator onboarding now provides a localized, responsive spotlight tour across the complete authoring workflow, remembers completion per client, preserves deep-link and workspace panel state, and can be replayed immediately from Settings.
...
- Production player surface evolution: the shipped Web runtime and Windows native host now use a dedicated responsive decision stage with reliable directional card transitions, direct click/keyboard/drag choices, immediate restart, shared Creator Skin tokens, build-scoped Reign session records, authored-language switching, project/About attribution, and DPI-aware native sizing. Continue with a full translation authoring matrix, broader interaction preferences, custom icons, signing, and additional platforms only through reviewed release work.
